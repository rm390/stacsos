...
class tarfs_file : public file {
 public:
  tarfs_file(tar_filesystem& fs, u64 data_start, u64 file_size)
      : file(file_size), fs_(fs), data_start_(data_start) {}
...
class tarfs_node : public fs_node {
 ...
 public:
  tarfs_node(filesystem& fs, fs_node* parent, fs_node_kind kind,
             const string& name, u64 data_start, u64 data_size)
  ...
  virtual shared_ptr<file> open() override {
    return shared_ptr<file>(
        new tarfs_file((tar_filesystem&)fs(), data_start_, data_size_));
  }
  virtual fs_node* mkdir(const char* name) override;

  // Custom fs_node function overrides for tarfs
  virtual size_t get_children(void* stats, size_t length) override;

 protected:
  virtual fs_node* resolve_child(const string& name) override;

 private:
  tarfs_node* add_child(const string& name, fs_node_kind kind, u64 data_start,
                        u64 data_size) {
...
class tar_filesystem : public physical_filesystem {
 ...
 private:
  void load_tree();
  void register_file(const tar_file_header* header, u64 data_block_start,
                     u64 data_size);
