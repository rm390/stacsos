...
class fs_node {
 public:
  fs_node(filesystem& parent_fs, fs_node* parent_node, fs_node_kind kind,
          const string& name)
      : fs_(parent_fs),
  ...
  // Custom fs_node functions, to be implemented by subclasses e.g tarfs-node class.
  virtual size_t get_children(void* stats, size_t length) { return 0; }
