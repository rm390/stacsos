...
// Get the children of the directory node, and stores in a stats array passed in
// as buffer
size_t tarfs_node::get_children(void* stats, size_t length) {
  //  dprintf("tarfs: get children\n");

  size_t stats_length = 0;
  info* stats_ptr = static_cast<info*>(stats);
  ...
  for (auto* child : children_) {
    ...
    // Copy the name, type, and size into the stats array
    memops::memcpy(stats_ptr[stats_length].name, name.c_str(), name_length);
    stats_ptr[stats_length].type =
        child->kind() == fs_node_kind::directory ? 'D' : 'F';
    stats_ptr[stats_length].size = child->data_size_;
...
void tar_filesystem::register_file(const tar_file_header* header,
                                   u64 data_block_start, u64 data_size) {
  ...
  while (!path_components.empty()) {
    ...
    if (path_components.empty()) {
      ...
      tarfs_node* existing_child =
          (tarfs_node*)current_node->resolve_child(component);
      if (existing_child != nullptr) {
        panic("file already exists");
      }

      current_node->add_child(component, fs_node_kind::file, data_block_start,
                              data_size);
      break;
    } else {
      ...
      if (existing_child == nullptr) {
        existing_child =
            current_node->add_child(component, fs_node_kind::directory, 0, 0);
      }
...
size_t tarfs_file::pread(void* buffer, size_t offset, size_t length) {
  ...
  while (length) {
    ...
    u64 amount_to_copy =
        min(length, (size_t)(sizeof(block_buffer) - start_block_offset));
    // dprintf("tarfs: atc: %lu, sbo: %lu\n", amount_to_copy, start_block_offset);

    memops::memcpy(output_ptr, block_buffer + start_block_offset,
                   amount_to_copy);
...
size_t tarfs_file::pwrite(const void* buffer, size_t offset, size_t length) {
  return 0;
}

void tarfs_file::read_file_blocks(void* buffer, u64 offset, u64 count) {
  fs_.bdev_.read_blocks_sync(buffer, data_start_ + offset, count);
}
