...
extern "C" syscall_result handle_syscall(syscall_numbers index, u64 arg0,
  ...
  switch (index) {
    // Custom syscall
    case syscall_numbers::read_dir: {
      // dprintf("Handling read_dir syscall\n");

      // Use the VFS to lookup the node for the given path
      auto node = vfs::get().lookup((const char*)arg0);
      ...
      // If the node is not a directory, return an error
      if (node->kind() != fs_node_kind::directory) {
        return syscall_result{syscall_result_code::not_supported, 0};
      }

      // Get the children of the directory node
      auto number_of_children = node->get_children((void*)arg1, arg2);

      // Return the number of children
      return syscall_result{syscall_result_code::ok, number_of_children};
    }
    ...
    case syscall_numbers::write: {
      ...
      return operation_result_to_syscall_result(
          o->write((const void*)arg1, arg2));
    }
    ...
    case syscall_numbers::pwrite: {
      ...
      return operation_result_to_syscall_result(
          o->pwrite((const void*)arg1, arg2, arg3));
    }
    ...
    case syscall_numbers::ioctl: {
      ...
      return operation_result_to_syscall_result(
          o->ioctl(arg1, (void*)arg2, arg3));
    }

    case syscall_numbers::alloc_mem: {
      auto rgn = current_thread.owner().addrspace().alloc_region(
          PAGE_ALIGN_UP(arg0), region_flags::readwrite, true);
    ...
    case syscall_numbers::start_process: {
      ...
      auto new_proc = process_manager::get().create_process((const char*)arg0,
                                                            (const char*)arg1);
      if (!new_proc) {
        return syscall_result{syscall_result_code::not_found, 0};
      }
      ...
      return syscall_result{
          syscall_result_code::ok,
          object_manager::get()
              .create_process_object(current_process, new_proc)
              ->id()};
    }
    ...
    case syscall_numbers::wait_for_process: {
      ...
      auto process_object =
          object_manager::get().get_object(current_process, arg0);
      if (!process_object) {
        return syscall_result{syscall_result_code::not_found, 0};
      }

      return operation_result_to_syscall_result(
          process_object->wait_for_status_change());
    }

    case syscall_numbers::start_thread: {
      auto new_thread =
          current_thread.owner().create_thread((u64)arg0, (void*)arg1);
      new_thread->start();

      return syscall_result{
          syscall_result_code::ok,
          object_manager::get()
              .create_thread_object(current_process, new_thread)
              ->id()};
    }
